...
class EventHub(object):
    """
        ...
        generates "complete" and "error" events based on them(see deploy and
        invoke operations). Clients of the sdk can register for Block and
        Chaincode events using the eventHub object. Clients can access the
        EventHub object created for a chain using the get_event_hub member of
        the Chain object or may create an EventHub object of their own.
        Each EventHub object created will create its own connection to the
        event source(usually a fabric peer).
    """
