...
if ! type configtxgen; then
    if  [[ ! -e fabric-bin/bin/configtxgen ]]; then
        ...
        binUrl+="/hyperledger/fabric/hyperledger-fabric/$platform-$project_version"
        binUrl+="/hyperledger-fabric-$platform-$project_version.tar.gz"
        if ! curl $binUrl | tar -C fabric-bin -vxz; then
