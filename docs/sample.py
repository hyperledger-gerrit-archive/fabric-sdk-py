...
if __name__ == "__main__":
    ...
    response = cli.channel_create(
        target_orderer='orderer.example.com',
    ...
    response = cli.channel_join(
        requestor=org1_admin,
        channel_name='businesschannel',
        target_peers=['peer0.org1.example.com', 'peer1.org1.example.com'],
        target_orderer='orderer.example.com')
    if response:
    ...
    response = cli.channel_join(
        ...
        channel_name='businesschannel',
        target_peers=['peer0.org2.example.com', 'peer1.org2.example.com'],
        target_orderer='orderer.example.com')
    if response:
        print("Join channel successful")
    else:
    ...
    response = cli.chaincode_instantiate(
        requestor=org1_admin,
        channel_name='businesschannel',
        target_peers=['peer0.org1.example.com'],
        args=args,
        cc_name='example_cc',
        cc_version='v1.0')
    if response:
