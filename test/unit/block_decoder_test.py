...
class BlockDecoderTest(unittest.TestCase):
    ...
    def test_decode_block_data(self):
        ...
        """
        data_keys = ['payload', 'signature']
        payload_header_keys = ['signature_header', 'channel_header']
        sh_keys = ['nonce', 'creator']
        ch_keys = ['extension', 'channel_id', 'timestamp', 'tx_id',
                   'epoch', 'version', 'type']
        sh_creator_keys = ['mspid', 'id_bytes']
        sh_nonce = b'cfa064bebe2a846b8f89a4fc82da2f652b9edb69eac65426'
        channel_id = b'testchainid'
        timestamp = b'2017-06-23 15:15:18'
        tx_id = \
            b'70add6a845ab8a90d97d402a6c0de665e717ed1bb74d37c6bf32232d8339194f'
        epoch = 0
        version = 1
        type_value = 1
        extension = b''
        ...
        self.assertListEqual(data_item.keys(), data_keys)
        self.assertListEqual(data_payload_header.keys(), payload_header_keys)
        self.assertListEqual(sh.keys(), sh_keys)
        self.assertListEqual(ch.keys(), ch_keys)
        self.assertListEqual(sh['creator'].keys(), sh_creator_keys)
