...
from hfc.fabric.block_decoder import FilteredBlockDecoder, BlockDecoder
...
class E2eTest(BaseTestCase):
    ...
    async def get_filtered_block_events(self):
        ...
        channel_event_hub.connect(start=0, stop=None, filtered=True)

        events = []
        reg_num, block_events = channel_event_hub.registerBlockEvent()
    ...
    async def get_full_block_events(self):
        ...
        channel = self.client.get_channel(self.channel_name)
        channel_event_hub = channel.newChannelEventHub(peer, org_admin)
        channel_event_hub.connect(start=0, stop=None, filtered=False)
        ...
        block = BlockDecoder().decode(events[2].block.SerializeToString())
        ...
        block = BlockDecoder().decode(
            events[len(events) - 1].block.SerializeToString())
