...
class QueryBlockTest(BaseTestCase):
    async def invoke_chaincode(self):
        ...
        join_req = await build_join_channel_req(org1, self.channel,
                                                self.client)
        await self.channel.join_channel(join_req)
        ...
        # wait for events
        args = self.client.get_peer_events(self.org1_admin, self.org1_peer,
                                           self.channel_name, tx_context.tx_id)
        try:
            await asyncio.wait_for(args, timeout=30)
    ...
    def test_query_block_success(self):
        ...
        tx_context = create_tx_context(self.org1_admin,
                                       ecies(),
                                       TXProposalRequest())
        responses, p, h = self.channel.query_block(tx_context,
                                                   [self.org1_peer], "1")
