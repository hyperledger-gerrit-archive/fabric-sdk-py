...
# https://jira.hyperledger.org/browse/FAB-7065?page=com.atlassian.jira.plugin.system.issuetabpanels%3Acomment-tabpanel&showAll=true
class zeroTarInfo(tarfile.TarInfo):
    def __init__(self, name=""):
        super(zeroTarInfo, self).__init__(name)

        self.uid = self.gid = 500
        self.mode = 100644
        self.mtime = 0
        self.pax_headers = {
            'atime': 0,
...
def package_chaincode(cc_path, cc_type=CC_TYPE_GOLANG):
    ...
    if cc_type == CC_TYPE_GOLANG:
        ...
        with zeroTimeContextManager():
            dist = tarfile.open(fileobj=tar_stream, mode='w|gz', tarinfo=zeroTarInfo)
            for dir_path, _, file_names in os.walk(proj_path):
                for filename in file_names:
                    file_path = os.path.join(dir_path, filename)
                    file_data = open(file_path, mode='rb').read()

                    tarinfo = zeroTarInfo()
                    dist.addfile(tarinfo, io.BytesIO(file_data))
