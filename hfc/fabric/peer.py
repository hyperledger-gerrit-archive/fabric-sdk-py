...
class Peer(object):
    ...
    def delivery(self, envelope, scheduler=None, filtered=False):
        ...
        if filtered:
            delivery_result = list(self._event_client.DeliverFiltered(iter([envelope])))
        else:
            delivery_result = list(self._event_client.Deliver(iter([envelope])))
    ...
    def get_events(self, tx_context, channel_name,
        ...
        kwargs = {}
        if self._tls_ca_certs_path:
            with open(self._tls_ca_certs_path, 'rb') as f:
                root_cert = f.read()
                kwargs['tls_cert_hash'] = base64.b64encode(root_cert)
                #kwargs['tls_cert_hash'] = root_cert
