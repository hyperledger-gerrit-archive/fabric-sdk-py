...
class Client(object):
    ...
    async def chaincode_instantiate(self, requestor, channel_name, peers,
        ...
        if wait_for_event:
            ...
            try:
                events = await asyncio.wait_for(asyncio.gather(*txEvents),
                                                timeout=wait_for_event_timeout)
            except asyncio.TimeoutError:
                for channel_event_hub in channel.getChannelEventHubsForOrg(
                        requestor):
                    channel_event_hub.unregisterTxEvent(tx_context_dep.tx_id)
    ...
    async def chaincode_invoke(self, requestor, channel_name, peers, args,
        ...
        if wait_for_event:
            ...
            for target_peer in target_peers:
                ...
                reg_id, chaincodeEvent = channel_event_hub.\
                    registerChaincodeEvent(cc_name, cc_pattern)
                chaincodeEvents.append(chaincodeEvent)
                channelEventsHubs[reg_id] = channel_event_hub

            try:
                events = await asyncio.wait_for(
                    asyncio.gather(*chaincodeEvents),
                    timeout=wait_for_event_timeout)
            except asyncio.TimeoutError:
                for reg_id in channelEventsHubs.keys():
                    channelEventsHubs[reg_id].unregisterChaincodeEvent(reg_id)
                raise TimeoutError('waitForEvent timed out.')
            except Exception as e:
                return str(e)
            else:
                ...
                # disconnect channel_event_hubs
                ceh = channel.getChannelEventHubsForOrg(requestor)
                for channel_event_hub in ceh:
                    channel_event_hub.disconnect()
    ...
    async def chaincode_query(self, requestor, channel_name, peers, args,
        ...
        # TODO should create a real transaction too
