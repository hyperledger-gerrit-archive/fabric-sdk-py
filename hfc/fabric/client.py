...
class Client(object):
    ...
    def chaincode_install(self, requestor, peer_names, cc_path,
    ...
    def chaincode_instantiate(self, requestor, peer_names, args,
                              cc_name, cc_version, timeout=5,
                              channel_name='businesschannel'):
        """
        Instantiate chaincode to given peers by requestor role
        ...
        response = utils.send_transaction(self.get_channel(
            channel_name).orderers, tran_req, tx_context)
    ...
    def chaincode_invoke(self, requestor, peer_names, args,
                         cc_name, cc_version, timeout=5,
                         channel_name='businesschannel'):
        """
        Invoke chaincode to given peers by requestor role
        ...
        # TODO: remove instantiate step in invoke
        res = self.send_instantiate_proposal(
            tx_context_dep, peers, channel_name)
        sleep(5)

        tran_req = utils.build_tx_req(res)
        utils.send_transaction(self.orderers, tran_req, tx_context)
        sleep(5)
