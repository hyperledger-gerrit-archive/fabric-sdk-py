...
"""
 ...
 to complete.
 As a result the applications must design their handling of the
 transaction lifecycle in an asynchronous fashion.
 After the transaction proposal has been successfully endorsed and before
 the transaction message has been successfully sent to the orderer, the
 application should register a listener to be notified when the transaction
 achieves finality, which is when the block containing the transaction gets
 added to the peer's ledger/blockchain.

 Fabric committing peers provide a block delivery service to publish blocks or
 filtered blocks to connected fabric-clients. See connect ChannelEventHub
 on connection options and how this ChannelEventHub may connect to the fabric
 service. For more information on the service see deliver
 (https://hyperledger-fabric.readthedocs.io/en/
  release-1.4/peer_event_services.html)
 ...
 interested listeners with the related contents of the block
 (e.g. transactionId, status).
 There are three types of listeners that will get notified by
 the fabric-client after it receives a published block from the fabric
 deliver service.
    A "block listener" gets called for every block received. The listener
    ...
    A "transaction listener" gets called when the specific transaction
     is committed (discovered inside a published block). The listener
     may also be registered to listen to "all" transactions.
     The listener will be passed the transaction id, transaction status and
     block number.
     See [registerTxEvent]{@link ChannelEventHub#registerTxEvent}
    A "chaincode event listener" gets called when a specific
   ...
   'all', // this listener will be notificed of all transactions
     ...
     (err) => {
        ...
        console.log(util.format('Error %s! Transaction listener has been ' +
                 'deregistered for %s', err, eh.get_peer_endpoint()));
     }
 );
...
class ChannelEventHub(object):
    ...
    def __init__(self, channel, peer):
        ...
        self._clientContext = channel.client
    ...
    def last_block_number(self):
        """
            ...
            :return: The block number of the last block seen
        """
    ...
    def _check_allow_registrations(self):
        """
    ...
    def set_peer_endpoint(self, peer_endpoint):
        """
            Set fabric event source (peer)

            Args:
                peer_endpoint (str): peer-id:peer-port
        """
        self._peer.endpoint(peer_endpoint)
    ...
    def connect(self, options, connectCallback):
        """
            ...
            If the connection fails to get established, the application will
            be notified via the error callbacks from the registerXXXEvent()
            methods. It is recommended that an application always register at
            least one event listener with an error callback, by calling any
            one of the registerBlockEvent ChannelEventHub#registerBlockEvent},
            registerTxEvent (ChannelEventHub#registerTxEvent) or
            registerChaincodeEvent(ChannelEventHub#registerChaincodeEvent)
            methods, before calling connect().

        Args:
            {ConnectOptions | boolean} options - Optional. If of type boolean
                then it will be assumed to how to connect to receive full
                (true) or filtered (false) blocks.
            {functon} connectCallback - Optional. This callback will report
                completion of the connection to the peer or  will report
                any errors encountered during connection to the peer. When
                there is an error, this ChannelEventHub will be shutdown
                (disconnected).
        ...
        # the following supports the users using the boolean parameter to
        # control how to connect to the fabric service for full or filtered
        # blocks
        ...
        if signedEvent is not None:
            signedEvent = self._validate_signed_event(signedEvent)

        if connectCallback:
            self.connectCallback = connectCallback

        _logger.debug('connect - start peerAddr:%s' % self.get_peer_endpoint())
        if (not self._clientContext._userContext and
                not self._clientContext._adminSigningIdentity and
                not signedEvent):
            raise Exception(
                'Error connect the ChannelEventhub to peer, either the' +
                'clientContext has not been properly initialized, missing' +
                'userContext or admin identity or missing signedEvent')

        if isinstance(full_block, bool):
            self._filtered_stream = not full_block
            _logger.debug(
                'connect - filtered block stream set to:%s'
                % self._filtered_stream)
        else:
        ...
        _logger.debug('connect - end %s' % self.get_peer_endpoint())
    ...
    def _connect(self, request):
        ...
        self._channel = create_grpc_channel(self._peer._endpoint,
                                            self._peer._tls_ca_certs_path,
                                            opts=options)
        ...
        if signedEvent:
            self._send_signed_registration(signedEvent)
        else:
    ...
    def _disconnect(self, err):
        ...
        _logger.debug('_disconnect - start -- called due to:: %s, peer:%s' % (
            str(err), self.get_peer_endpoint()))
        self._connected = False
        self._connect_running = False
        self._close_all_callbacks(err)
        self._shutdown()
        self._set_replay_defaults()

        # one last thing, report to the connect callback
        if self.connectCallback:
            # report and ourselves so user will know the source
            self.connectCallback(err, self)
            self.connectCallback = None
    ...
    def _send_registration(self):
        ...
        seekPayloadBytes = self.generate_unsigned_registration(opt)
    ...
    def _validate_signed_event(self, signedEvent):
        """Internal method
        ...
        method = '_validate_signed_event'
        _logger.debug('%s - enter' % method)

        if 'signature' not in signedEvent:
            raise Exception('Empty signature in signed event')

        if 'payload' not in signedEvent:
    ...
    def generate_unsigned_registration(self, options):
        ...
        # either we have both identity and txId, or we have both certificate
        # or mspId
        if identity or txId:
            if not txId:
                raise Exception(
                    '"options.txId" is required to generate unsigned \
                    fabric service registration')
            if not identity:
                raise Exception(
                    '"options.identity" is required to generate unsigned \
                    fabric service registration')
        ...
        if certificate or mspId:
            if not certificate:
                raise Exception(
                    '"options.certificate" is required to generate \
                    unsigned fabric service registration')
            if not mspId:
                raise Exception(
                    '"options.mspId" is required to generate unsigned\
                     fabric service registration')
    ...
    def _close_all_callbacks(self, err):
        """Internal method
            Will close out all callbacks
            Sends an error to all registered event "onError" callbacks
        """

        method = '_close_all_callbacks - %s' % self.get_peer_endpoint()
        ...
        def cc_closer(key):
            ...
            for chaincode_reg in cbtable:
                _logger.debug(
                    '%s - closing this chaincode event ccid:%s \
                    eventNameFilter:%s' % (
    ...
    def _check_replay(self, options):
        """Internal method
        ...
        if (have_start_block or have_end_block) and self._haveRegistrations():
            _logger.error(
                'This ChannelEventHub is already registered with active \
                listeners. Not able options of startBlock:%s endBlock:%s' % (
                    options.startBlock, options.endBlock))
            raise Exception(
                'Only one event registration is allowed when startBlock \
                or endBlock are used')
        if (have_start_block or have_end_block) and (
                self._connected or self._connect_running):
            _logger.error(
                'This ChannelEventHub has already been connected to start \
                receiving blocks. Not able to use options of \
                startBlock:%s endBlock:%s' % (
                    options.startBlock, options.endBlock))
            raise Exception(
                'Event listeners that use startBlock or endBlock must be \
                registered before connecting to the peer channel-based\
                 service')
        ...
        if have_end_block:
            if have_start_block and converted_options['start_block'] > \
                ...
                raise Exception(
                    '`"startBlock" (%s) must not be larger than \
                    "endBlock" (%s})' % (
    ...
    def _check_connection(self):
        ...
        if self._connected or self._connect_running:
            ready = is_stream_ready(self)
            _logger.debug(
                '_checkConnection - %s with stream channel ready %s'
                % self._peer.getUrl(),
                ready)

            if not ready and not self._connect_running:  # Not READY, trying
    ...
    def check_connection(self, force_reconnect):
        ...
        ready = is_stream_ready(self)
        _logger.debug(
            'checkConnection - %s with stream channel ready %s'
            % self._peer.getUrl(),
            ready)
        ...
        if force_reconnect:
            try:
                ...
                err = Exception(
                    'Problem during reconnect and the event hub is not \
                    connected ::%s' % str(
                        error))
                self._disconnect(err)

        return is_stream_ready(self)
    ...
    def register_chaincode_event(self, ccid, eventname, onEvent, onError,
        ...
        self._check_allow_registrations()
        default_disconnect = False

        startstop_mode = self._check_replay(options)
    ...
    def unregister_chaincode_event(self, listener_handle, raiseError=None):
        ...
        if not cbtable and raiseError:
            raise Exception(
                'No event registration for chaincode id %s'
                % listener_handle.ccid)
        elif cbtable:
    ...
    def register_block_event(self, onEvent, onError, options):
        """
            ...
            The listener's "onEvent" callback gets called on the arrival of
            every block.

            An error may occur in the connection establishment which runs
            asynchronously. The best practice would be to provide an
            "onError" callback to be notified when this ChannelEventHub has
            an issue.
        """
    ...
    def unregister_block_event(self, block_registration_number,
        ...
        if not block_reg and raiseError:
            raise Exception(
                'Block listener for block registration number "%s" \
                does not exist' % block_registration_number)
        else:
    ...
    def unregister_tx_event(self, txid, raiseError=None):
        ...
        if not tx_reg and raiseError:
            raise Exception(
                'Transaction listener for transaction id "%s" does not exist'
                % txid)
        else:
            del self._transactionRegistrations[txid]

    def _process_block_events(self, block):
        """
    ...
    def _process_tx_events(self, block):
        """
        ...
        if len(self._transactionRegistrations.keys()) == 0:
            _logger.debug(
                '_processTxEvents - no registered transaction event \
                "listeners"')
            return
        ...
        if block.number:
            ...
            if block.filtered_transactions:
                for filtered_transaction in block.filtered_transactions:
                    tx_validation_code = filtered_transaction.\
                        tx_validation_code
                    self._check_transaction_id(filtered_transaction.txid,
                                               tx_validation_code,
                                               block.number)
        else:
            _logger.debug(
            ...
            while index < block.data.data.length:
                ...
                self._check_transaction_id(channel_header.tx_id,
                                           txStatusCodes[index],
                                           block.header.number)

    def _check_transaction_id(self, tx_id, val_code, block_num):
        """
        ...
        if trans_reg:
            self._call_transaction_listener(tx_id, val_code, block_num,
                                            trans_reg)
        ...
        if trans_reg or all_trans_reg:
            _logger.debug(
                '_callTransactionListener - no call backs found for \
                this transaction %s' % tx_id)

    def _call_transaction_listener(self, tx_id, val_code, block_num,
                                   trans_reg):
        """
            internal utility method
        """
        _logger.debug(
            '_callTransactionListener - about to call the transaction \
            call back for code=%s tx=%s' % (
                val_code, tx_id))
        status = convert_validation_code(val_code)
        trans_reg.onEvent(tx_id, status, block_num)
        if trans_reg.unregister:
            self.unregisterTxEvent(tx_id)
            _logger.debug(
                '_callTransactionListener - automatically unregister \
                tx listener for %s' % tx_id)
    ...
    def _process_chaincode_events(self, block):
        """
        ...
        if len(self._chaincodeRegistrants.keys()) == 0:
            _logger.debug(
                '_processChaincodeEvents - no registered chaincode event \
                "listeners"')
            return
        ...
        if block.number:
            if block.filtered_transactions:
                for filtered_transaction in block.filtered_transactions:
                    if (filtered_transaction.transaction._actions and
                            filtered_transaction.transaction_actions.
                            chaincode_actions):
                        for chaincode_action in filtered_transaction.\
                                transaction_actions.chaincode_actions:
                            self._call_chaincode_listener(
                    ...
                    if channel_header.type == 3:
                        ...
                        if tx and tx.actions:
                            for payload in tx.actions:
                                chaincode_event = payload.action.\
                                    proposal_response_payload.extension.events
                                _logger.debug(
                            ...
                            _logger.debug(
                                '_processChaincodeEvents - no transactions \
                                or transaction actions')
                    else:
                        _logger.debug(
                            '_processChaincodeEvents - block is not endorser \
                            transaction type')
    ...
    def _call_chaincode_listener(self, chaincode_event, block_num, tx_id,
                                 val_code, filtered):
        """
            private internal method for processing chaincode events
        """
        ...
        if not cbtable:
            _logger.debug(
                '_call_chaincode_listener - no chaincode listeners found')
            return

        tx_status = convert_validation_code(val_code)
        _logger.debug('_call_chaincode_listener - txid=%s  val_code=%s' % (
            tx_id, tx_status))

        for chaincode_reg in cbtable:
            if chaincode_event.event_name in chaincode_reg.eventNameFilter:
                _logger.debug(
                    '_call_chaincode_listener - calling chaincode listener \
                    callback')
                if filtered:
                ...
                if chaincode_reg.event_reg.unregister:
                    ...
                    _logger.debug(
                        '_call_chaincode_listener - automatically unregister\
                         tx listener for %s' % tx_id)
                if chaincode_reg.event_reg.disconnect:
                    self._disconnect(Exception(
                        'Shutdown due to disconnect on transaction id\
                         registration'))
            else:
                _logger.debug(
                    '_call_chaincode_listener - NOT calling chaincode \
                    listener callback')
    ...
    def _checkReplayEnd(self):
        ...
        """
        if (self._ending_block_number and
                self._ending_block_number <= self._last_block_seen):
...
def convert_validation_code(code):
...
def is_stream_ready(obj):
    method = 'is_stream_ready'
    ready = False
    if obj._stream:
        stream = obj._stream
        ready = stream.readable and stream.writable and stream.reading
        _logger.debug('%s - stream.readable %s :: %s' % (
            method, stream.readable, obj.get_peer_endpoint()))
        _logger.debug('%s - stream.writable %s :: %s' % (
            method, stream.writable, obj.get_peer_endpoint()))
        _logger.debug('%s - stream.reading %s :: %s' % (
            method, stream.reading, obj.get_peer_endpoint()))
        _logger.debug('%s - stream.read_status %s :: %s' % (
            method, stream.read_status, obj.get_peer_endpoint()))
        _logger.debug('%s - stream.received_status %s :: %s' % (
            method, stream.received_status, obj.get_peer_endpoint()))

    return ready


# The ChaincodeRegistration is used internal to the ChannelEventHub to
# hold chaincode event registration callbacks.
    ...
    def __init__(self, onEvent, onError, options, default_unregister,
        ...
        if options:
            if not options.unregister:
                ...
                raise Exception(
                    'Event registration has invalid value for \
                    "unregister" option')
            ...
            if not options.disconnect:
                ...
                raise Exception(
                    'Event registration has invalid value for "disconnect"\
                     option')

    def onEvent(self, *args):
        try:
            self._onEventFn(*args)
        except Exception as error:
            _logger.warning(
