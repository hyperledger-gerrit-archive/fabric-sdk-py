...
class ChannelEventHub(object):
    ...
    async def _registerTxEvent(self, tx_id):
    ...
    def registerTxEvent(self, tx_id):
        self._tx_ids.append(tx_id)
        return tx_id, self._registerTxEvent(tx_id)
    ...
    async def _handle_chaincode_events(self, reg_id, ccid, pattern):
        async for event in self._events:
            ...
            if self._filtered:
                evt = FilteredBlockDecoder().decode(
                    event.filtered_block.SerializeToString())
            else:
                evt = BlockDecoder().decode(event.block.SerializeToString())

            if 'data' in evt:
                data = evt['data']['data'][0]['payload']['data']
                action = data['actions'][0]['payload']['action']
                ppl_r_p = action['proposal_response_payload']
                block_events = ppl_r_p['extension']['events']
                if block_events['chaincode_id'] == ccid and\
                        re.match(pattern, block_events['event_name']):
                    return evt
