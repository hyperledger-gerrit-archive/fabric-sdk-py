...
class Channel(object):
    ...
    def getChannelEventHubsForOrg(self, requestor, org_name=None):
        if org_name:
            channel_event_hubs = self._channel_event_hubs.get(org_name, [])
        else:
            channel_event_hubs = self._channel_event_hubs.get(requestor.org,
                                                              [])

        return [x for x in channel_event_hubs if x._connected]
