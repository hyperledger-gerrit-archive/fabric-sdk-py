...
class Channel(object):
    ...
    def _get_block_by_number(self, tx_context, orderer, block_number):
        """
        Args:
            tx_context: tx_context instance
            orderer: orderer instance
            block_number: number of block to query for

        Returns: block
        """
        seek_info = create_seek_info(block_number, block_number)
        ...
        if res.block is None or res.block == '':
            _logger.error("fail to get block_number=%s", str(block_number))
            return None

        _logger.info("get block successfully, block=%s",
                     res.block.header)
        return res.block
