...
class Channel(object):
    ...
    async def join_channel(self, request):
        """
        ...
        Return:
            True in success or Exception in failure
        """
        ...
        if all([x.response.status == 200 for x in res]):
            _logger.info("All peers successfully joined the channel")
            return True

        raise IOError("fail to send transaction proposal: %s" % res)
